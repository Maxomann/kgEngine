...
namespace kg
{
	...
	public:
		...
		template< class T >
		CORE_API T& reloadResourceFromResourceFolder( std::string relativePath )
		{
			return reloadResource<T>( resourceFolderPath + relativePath );
		}
		...
		template< class T >
		CORE_API T& reloadResourceFromResourceFolderForTile( const int tileID, std::string fileExtension )
		{
			return reloadResource<T>( resourceFolderPath + "tile" + std::to_string( tileID ) + fileExtension );
		}
		...
		template< class T >
		CORE_API T& reloadResource( std::string path )
		{
			const size_t typeID = typeid(T).hash_code();
